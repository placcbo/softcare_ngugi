...
import { PARTS, CATEGORIES, BRANDS, type PartCategory, type Brand } from "@/lib/inventory";
      ...
      <Section className="!py-8">
        ...
        <div className="glass rounded-2xl p-4 sm:p-5">
          <label className="relative block">
            ...
            />
            {q && (
              <button onClick={() => setQ("")} aria-label="Clear" className="absolute right-3 top-1/2 -translate-y-1/2 rounded-md p-1 text-muted-foreground hover:text-foreground">
                <X className="h-4 w-4" />
              </button>
            )}
          </label>
          <div className="mt-3 flex flex-wrap items-center gap-2">
            ...
            {SUGGESTIONS.map((s) => (
              <button key={s} onClick={() => setQ(s)} className="rounded-full border border-border bg-card px-3 py-1 text-xs font-medium text-muted-foreground hover:border-primary hover:text-foreground">
                {s}
              </button>
            ))}
          </div>
          ...
          <div className="mt-4 grid gap-3 sm:grid-cols-3">
            ...
            <select value={brand} onChange={(e) => setBrand(e.target.value as Brand | "All")}
                    className="rounded-md border border-border bg-background px-3 py-2 text-sm">
        ...
        {results.length === 0 ? (
          <div className="mt-8 rounded-2xl border border-dashed border-border p-10 text-center">
            ...
            <a href={waLink(`Hi! Do you have this part in stock: "${q}"?`)} target="_blank" rel="noreferrer"
              ...
              <article key={p.id} className="card-hover overflow-hidden rounded-2xl border border-border bg-card">
                ...
                <div className="p-5">
                  ...
                  <div className="mt-4 flex items-center justify-between">
                    ...
                    <a href={waLink(`Hi! Is the ${p.name} (KSh ${p.price.toLocaleString()}) available?`)} target="_blank" rel="noreferrer"
