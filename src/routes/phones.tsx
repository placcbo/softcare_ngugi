...
const Route = createFileRoute("/phones")({
  head: () => ({
    meta: [
...
const FILTERS = ["All", "New", "Refurbished"];
function PhonesPage() {
  ...
  >
        ...
        <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {phones.map((p) => <article key={p.id} className="card-hover overflow-hidden rounded-2xl border border-border bg-card">
              <div className="bg-gradient-to-b from-secondary to-background">
                <img src={p.image} alt={p.name} loading="lazy" width={800} height={1e3} className="mx-auto aspect-[4/5] w-full object-contain p-6" />
              </div>
              <div className="p-5">
                ...
                <div className="mt-4 flex items-center justify-between">
                  ...
                  <a
    href={waLink(`Hi! I'm interested in the ${p.name} (KSh ${p.price.toLocaleString()}).`)}
    target="_blank"
    rel="noreferrer"
